    ...
    public function rules()
    {
        ...
        if (in_array($this->method(), ['PUT', 'PATCH'])) {
            ...
            foreach (config('translatable.locales') as $locale) {
                $rules += [$locale . '.title' => ['required','max:100', Rule::unique('category_translations', 'title')->ignore($category->id, 'category_id')]];
            }//end of for each
        }//end of if    
        else{
            foreach (config('translatable.locales') as $locale) {
                $rules += [$locale . '.title' => ['required','max:100', Rule::unique('category_translations', 'title')]];
            }//end of for each
        }       
        return $rules;
    }
